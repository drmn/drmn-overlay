...
SRC_URI="https://commondatastorage.googleapis.com/chromium-browser-official/${C_P}.tar.xz
	...
	!system-toolchain? (
		...
		https://commondatastorage.googleapis.com/chromium-browser-clang/Linux_x64/rust-toolchain-${GOOGLE_RUST_VER}-${GOOGLE_CLANG_VER%???}.tar.xz
			-> chromium-${C_PV%%\.*}-rust.tar.xz
	)
	test? (
		https://commondatastorage.googleapis.com/chromium-browser-official/${C_P}-testdata.tar.xz
		https://chromium-fonts.storage.googleapis.com/${TEST_FONT} -> chromium-${C_PV%%\.*}-testfonts.tar.gz
		)
	ppc64? (
...
src_unpack() {
	...
	if use system-toolchain; then
		...
		unpack chromium-${C_PV%%\.*}-clang.tar.xz
...
src_prepare() {
	...
	# Directory contents are already pruned by the ungoogled patches
